...
print(f"   TLS_SCANNER_URL: {TLS_SCANNER_URL}")
print(f"   REPO_SCANNER_URL: {REPO_SCANNER_URL}")
print(f"   MAX_CONCURRENT_SCANS: {MAX_CONCURRENT_SCANS}")
...
async def fetch_repo_branches(owner: str, repo_name: str, client: httpx.AsyncClient) -> List[str]:
    """Fetch all branch names for a given repository."""
    try:
        url = f"{GITHUB_API_BASE}/repos/{owner}/{repo_name}/branches"
        response = await client.get(url, timeout=GITHUB_API_TIMEOUT)
        response.raise_for_status()
        branches_data = response.json()
        return [branch['name'] for branch in branches_data]
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        print(f"Could not fetch branches for {owner}/{repo_name}: {e}")
        return []
...
async def discover_github_repos(request: GitHubDiscoveryRequest):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            repos_response = await client.get(repos_url, timeout=GITHUB_API_TIMEOUT)
            repos_response.raise_for_status()
            repos_data = repos_response.json()
            
            public_repos = [repo for repo in repos_data if not repo.get('fork', False)]

            # Concurrently fetch branches for all public repos
            branch_fetch_tasks = [fetch_repo_branches(username, repo['name'], client) for repo in public_repos]
            branches_results = await asyncio.gather(*branch_fetch_tasks, return_exceptions=True)

            discovered_repos = []
            for repo, branches in zip(public_repos, branches_results):
                repo_model = GitHubRepo(
                    ...
                    branches=branches if not isinstance(branches, Exception) else []
                )
...
async def batch_repo_scan_from_github(
    ...
    if not request.repos:
        raise HTTPException(status_code=400, detail="No repositories provided to scan.")
...
async def batch_tls_scan(
    ...
    try:
        ...
        if 'domain' not in df.columns:
            raise HTTPException(
                status_code=400, 
                detail=f"Excel file must contain 'domain' column. Found columns: {df.columns.tolist()}"
            )
        ...
        import traceback
        print(f"Error processing Excel file: {traceback.format_exc()}")  # Full error trace for debugging
...
async def scan_single_repository(repo_url: str, branch_name: str) -> ScanResult:
    ...
    """
    # Use a longer timeout to accommodate slow clones on the server side
    async with httpx.AsyncClient(timeout=90.0) as client:
        ...
        try:
            ...
            try:
                scans_response = await client.get(f"{REPO_SCANNER_URL}/api/scans")
                ...
                # If recovery also fails, we'll proceed to the final failure case
                pass
        
        # 3. If we have a repo_id, poll for its final status
        if repo_id:
            max_attempts = 600  # Poll for up to 30 minutes
            for i in range(max_attempts):
                await asyncio.sleep(3)
                try:
                    status_response = await client.get(f"{REPO_SCANNER_URL}/api/scans/{repo_id}")
                    ...
                    if status.get("scan_status") == "failed":
                        return ScanResult(
                            domain_or_repo=repo_url,
                            status="failed",
                            error=status.get("current_status", "Scan failed in worker"),
                            timestamp=datetime.utcnow()
                        )
                except (httpx.HTTPError, httpx.TimeoutException) as poll_e:
...
async def batch_repo_scan(
    ...
    try:
        ...
        if 'repo_url' not in df.columns:
            raise HTTPException(
                status_code=400,
                detail=f"Excel file must contain 'repo_url' column. Found columns: {df.columns.tolist()}"
            )
        ...
        if not repositories:
            raise HTTPException(status_code=400, detail="No valid repositories found in Excel file")
